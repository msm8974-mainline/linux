...
static int otm1902b_on(struct otm1902b *ctx)
{
	...
	mipi_dsi_usleep_range(&dsi_ctx, 1000, 2000);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xff, 0x19, 0x02);
	mipi_dsi_usleep_range(&dsi_ctx, 1000, 2000);
...
{
	return drm_connector_helper_get_modes_fixed(connector, &otm1902b_mode);
}
...
static int otm1902b_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx = devm_drm_panel_alloc(dev, struct otm1902b, panel,
				   &otm1902b_panel_funcs,
