...
static struct snd_soc_component *left_component;
static struct snd_soc_component *right_component;
				...
				u8 *value, u8 len)
{
	...
	printk("tfa write 0x%x",tfa9890_client->addr);
	do {
		err = i2c_transfer(tfa9890_client->adapter, msgs,
					ARRAY_SIZE(msgs));
		if (err != ARRAY_SIZE(msgs))
...
static int tfa9890_bulk_write(struct snd_soc_component *component, unsigned int reg,
				const void *data, size_t len)
{
	struct tfa9890_priv *tfa9890 = snd_soc_component_get_drvdata(component);
	...
	while ((remaining_bytes > 0)) {
		...
		ret = tfa9890_i2c_write(tfa9890->control_data, chunk_buf,
					chunk_size + 1);
...
static int tfa9890_write(struct snd_soc_component *component, unsigned int reg,
				unsigned int val)
{
	struct tfa9890_priv *tfa9890 = snd_soc_component_get_drvdata(component);
	u8 buf[3] = {0, 0, 0};
	...
	mutex_lock(&tfa9890->i2c_rw_lock);
	ret = tfa9890_i2c_write(tfa9890->control_data, buf, ARRAY_SIZE(buf));
	mutex_unlock(&tfa9890->i2c_rw_lock);
	return ret;
}

static unsigned int tfa9890_read(struct snd_soc_component *component, unsigned int reg)
{
	struct tfa9890_priv *tfa9890 = snd_soc_component_get_drvdata(component);
	u8 buf[3] = {0, 0, 0};
	int len;
	...
	if (tfa9890_i2c_read(tfa9890->control_data, reg & 0xff, buf, len) == 0) {
		if (len == 3)
...
static int tfa9890_bulk_read(struct snd_soc_component *component, u8 reg,
				u8 *data, int len)
{
	struct tfa9890_priv *tfa9890 = snd_soc_component_get_drvdata(component);
	int ret = 0;
	...
	while ((remaining_bytes > 0)) {
		if (remaining_bytes < chunk_size)
			chunk_size = remaining_bytes;
		ret = tfa9890_i2c_read(tfa9890->control_data, reg, data + offset,
				chunk_size);
		offset = offset + chunk_size;
		remaining_bytes = remaining_bytes - chunk_size;
	}
...
static int tfa9890_dsp_transfer(struct snd_soc_component *component,
			...
			int len, int type, u8 *read)
{
	...
	buffer[7] = param_id;
	err = tfa9890_bulk_write(component, TFA9890_CF_CONTROLS, buffer,
						ARRAY_SIZE(buffer));
	if (err < 0) {
	...
	if (type == TFA9890_DSP_WRITE) {
		...
		err = tfa9890_bulk_write(component, TFA9890_CF_MEM, bytes, len);
		if (err < 0)
			return err;
	}
	...
	snd_soc_component_write(component, TFA9890_CF_CONTROLS, cf_ctrl);

	/* wait for ~1 msec (max per spec) and check for DSP status */
	msleep_interruptible(1);
	cf_status = snd_soc_component_read32(component, TFA9890_CF_STATUS);
	...
	err = tfa9890_bulk_write(component, TFA9890_CF_CONTROLS, buffer, 5);
	if (err < 0) {
	...
	rpc_status = snd_soc_component_read32(component, TFA9890_CF_MEM);
	...
	if (type == TFA9890_DSP_READ) {
		...
		snd_soc_component_write(component, TFA9890_CF_MAD, cf_mad);
		tfa9890_bulk_read(component, TFA9890_CF_MEM,
				read, len);
	}
...
static int tfa9890_set_config_right(struct tfa9890_priv *tfa9890)
{
	...
	ret = tfa9890_dsp_transfer(tfa9890->component,
...
static int tfa9890_set_mode_right(struct tfa9890_priv *tfa9890)
{
	...
	ret = tfa9890_dsp_transfer(tfa9890->component, TFA9890_DSP_MOD_SPEAKERBOOST,
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (tfa9890->curr_mode != mode_value ||
		...
		val = snd_soc_component_read32(component, TFA9890_SYS_STATUS_REG);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0]) {
		...
		val = snd_soc_component_read32(component, TFA9890_I2S_CTL_REG);
		val &= ~(TFA9890_I2SREG_CHSA_MSK);
		snd_soc_component_write(component, TFA9890_I2S_CTL_REG, val);

		/* Set DCDC compensation to off and set impedance as 8ohm */
		val = snd_soc_component_read32(component, TFA9890_SYS_CTL2_REG);
		val &= ~(TFA9890_SYS_CTRL2_REG_DCFG_MSK);
		val |= TFA9890_SYS_CTRL2_REG_SPKR_MSK;
		snd_soc_component_write(component, TFA9890_SYS_CTL2_REG, val);

		/* Set DCDC to follower mode and disable coolflux  */
		val = snd_soc_component_read32(component, TFA9890_SYS_CTL1_REG);
		val &= ~(TFA9890_SYS_CTRL_DCA_MSK);
		val &= ~(TFA9890_SYS_CTRL_CFE_MSK);
		snd_soc_component_write(component, TFA9890_SYS_CTL1_REG, val);

		/* Bypass battery safeguard */
		val = snd_soc_component_read32(component, TFA9890_BATT_CTL_REG);
		val |= TFA9890_BAT_CTL_BSSBY_MSK;
		snd_soc_component_write(component, TFA9890_BATT_CTL_REG, val);
 ...
 */
static int tfa9890_coldboot(struct snd_soc_component *component)
{
	...
	ret = tfa9890_bulk_write(component, TFA9890_CF_CONTROLS, coldpatch,
...
static void tfa9890_power(struct snd_soc_component *component, int on)
{
	u16 val;
...
static void tfa9890_set_mute(struct snd_soc_component *component, int mute_state)
{
	u16 mute_val;
	u16 amp_en;

	/* read volume ctrl register */
	mute_val = snd_soc_component_read32(component, TFA9890_VOL_CTL_REG);
	/* read sys ctrl register */
	amp_en = snd_soc_component_read32(component, TFA9890_SYS_CTL1_REG);
	...
	snd_soc_component_write(component, TFA9890_VOL_CTL_REG, mute_val);
	snd_soc_component_write(component, TFA9890_SYS_CTL1_REG, amp_en);
}
...
static int tfa9890_get_ic_ver(struct snd_soc_component *component)
{
	u16 ver1;
	u16 ver2;
	u16 ver3;
	...
	ver1 = snd_soc_component_read32(component, 0x8b) & TFA9890_N1B12_VER1_MASK;
	ver2 = snd_soc_component_read32(component, 0x8c);
	ver3 = snd_soc_component_read32(component, 0x8d) & TFA9890_N1B12_VER3_MASK;
...
static int tfa9890_wait_pll_sync(struct tfa9890_priv *tfa9890)
{
	...
	int tries = 0;
	u16 val;
	struct snd_soc_component *component = tfa9890->component;
	...
	if (tries == PLL_SYNC_RETRIES) {
		pr_err("tfa9890:DSP pll sync failed!! %s", component->name);
		ret = -EIO;
	}
...
static int tfa9887_load_dsp_patch(struct snd_soc_component *component, const u8 *fw,
				int fw_len)
{
	int index = 0;
	int length;
	...
	while (index < length) {
		...
		err = tfa9890_bulk_write(component, *(fw_data + index),
				(fw_data + index), size);
		if (err < 0) {
...
static int tfa9890_load_config(struct tfa9890_priv *tfa9890)
{
	...
	if (fw_patch) {
		ret = tfa9887_load_dsp_patch(component, fw_patch->data,
				fw_patch->size);
		if (ret) {
	...
	ret = request_firmware(&fw_speaker, fw_name, component->dev);
	if (ret) {
	...
	ret = tfa9890_dsp_transfer(component, TFA9890_DSP_MOD_SPEAKERBOOST,
	...
	val = (u16)snd_soc_component_read32(component, TFA9890_SYS_CTL1_REG);
...
static void tfa9890_calibaration(struct tfa9890_priv *tfa9890)
{
	 ...
	 * amp enabled*/
	tfa9890_set_mute(component, TFA9890_DIGITAL_MUTE);

	/* unlock write access MTP memory*/
	snd_soc_component_write(component, TFA9890_MTP_KEY_REG, TFA9890_MTK_KEY);

	val = snd_soc_component_read32(component, TFA9890_MTP_REG);
	/* set MTPOTC = 1 & MTPEX = 0*/
	val = val | TFA9890_MTPOTC;
	val = val & (~(TFA9890_STATUS_MTPEX));
	snd_soc_component_write(component, TFA9890_MTP_REG, val);

	/* set CIMTB to initiate copy of calib values */
	val = snd_soc_component_read32(component, TFA9890_MTP_COPY_REG);
	val = val | TFA9890_STATUS_CIMTP;
	snd_soc_component_write(component, TFA9890_MTP_COPY_REG, val);
}
...
static void tfa9890_work_read_imp(struct work_struct *work)
{
	...
	mutex_lock(&lr_lock);
	val = snd_soc_component_read32(tfa9890->component, TFA9890_SYS_STATUS_REG);
...
static void tfa9890_dsp_init(struct work_struct *work)
{
	...
	ret = tfa9890_coldboot(component);
	if (ret < 0) {
...
static int tfa9890_set_dai_sysclk(struct snd_soc_dai *component_dai,
...
static int tfa9890_set_dai_fmt(struct snd_soc_dai *component_dai, unsigned int fmt)
{
...
static int tfa9890_mute(struct snd_soc_dai *dai, int mute)
{
	...
	mutex_lock(&tfa9890->dsp_init_lock);
	if (mute) {
		tfa9890_set_mute(component, TFA9890_AMP_MUTE);
		do {
		...
		if (tfa9890->dsp_init == TFA9890_DSP_INIT_PENDING) {
			 ...
			 */
			snd_soc_component_write(component, TFA9890_CF_CONTROLS, 0x1);
			/* power up IC */
			tfa9890_power(component, 1);
			...
			snd_soc_component_write(component, TFA9890_CF_CONTROLS, 0x0);
		}

		tfa9890_set_mute(component, TFA9890_MUTE_OFF);
				   ...
				   struct snd_soc_dai *dai)
{
	pr_err("%s",__func__);
	struct snd_soc_component *component = dai->component;
	struct tfa9890_priv *tfa9890 = snd_soc_component_get_drvdata(dai->component);
...
/* Called from tfa9890 stereo component stub driver to set mute
 * on the individual codec, this is needed to smother pops
...
{
	pr_err("%s",__func__);
	struct tfa9890_priv *tfa9890_left = snd_soc_component_get_drvdata(left_component);
	struct tfa9890_priv *tfa9890_right = snd_soc_component_get_drvdata(right_component);
	u16 left_val;
	u16 right_val;
	u16 tries = 0;
	...
	mutex_lock(&lr_lock);
	if (mute) {
		tfa9890_set_mute(left_component, TFA9890_AMP_MUTE);
		tfa9890_set_mute(right_component, TFA9890_AMP_MUTE);
		do {
			/* need to wait for amp to stop switching, to minimize
			 * pop, else I2S clk is going away too soon interrupting
			 * the dsp from smothering the amp pop while turning it
			 ...
			 * switching to stop.
			 */
			left_val = snd_soc_component_read32(left_component,
							TFA9890_SYS_STATUS_REG);
			right_val = snd_soc_component_read32(right_component,
							TFA9890_SYS_STATUS_REG);
				  ...
				  char *buf, loff_t off, size_t count)
{
	...
	if (tfa9890->component) {
		...
		if ((val & TFA9890_STATUS_PLLS) &&
				(val & TFA9890_STATUS_CLKS)) {
			tfa9890_dsp_transfer(tfa9890->component,
				TFA9890_DSP_MOD_SPEAKERBOOST,
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (tfa9890->component) {
		...
		if ((val & TFA9890_STATUS_PLLS) &&
			 ...
			 */
			val = snd_soc_component_read32(tfa9890->component,
...
static int tfa9890_probe(struct snd_soc_component *component)
{
	struct tfa9890_priv *tfa9890 = snd_soc_component_get_drvdata(component);
	int i;
	u16 val;

	/* set component Bulk write method, will be used for
	 * loading DSP firmware and config files.
	 */
	tfa9890->component = component;
	/* reset registers to Default values */
	snd_soc_component_write(component, TFA9890_SYS_CTL1_REG, 0x0002);

	/* initialize I2C registers */
	for (i = 0; i < ARRAY_SIZE(tfa9890_reg_defaults); i++) {
		snd_soc_component_write(component, tfa9890_reg_defaults[i].reg,
	...
	pr_info("%s codec registered", component->name);
	return 0;
}

static void tfa9890_remove(struct snd_soc_component *component)
{
	tfa9890_power(component, 0);
}

static struct snd_soc_component_driver soc_component_dev_tfa9890_l = {
	.read = tfa9890_read,
	.write = tfa9890_write,
	.probe = tfa9890_probe,
	...
	.controls = tfa9890_left_snd_controls,
	.num_controls = ARRAY_SIZE(tfa9890_left_snd_controls),
	.dapm_widgets = tfa9890_left_dapm_widgets,
	.num_dapm_widgets = ARRAY_SIZE(tfa9890_left_dapm_widgets),
	.dapm_routes = tfa9890_left_dapm_routes,
	.num_dapm_routes = ARRAY_SIZE(tfa9890_left_dapm_routes),
};

static struct snd_soc_component_driver soc_component_dev_tfa9890_r = {
	.read = tfa9890_read,
	.write = tfa9890_write,
	.probe = tfa9890_probe,
	.remove = tfa9890_remove,
	...
	.controls = tfa9890_right_snd_controls,
	.num_controls = ARRAY_SIZE(tfa9890_right_snd_controls),
	.dapm_widgets = tfa9890_right_dapm_widgets,
	.num_dapm_widgets = ARRAY_SIZE(tfa9890_right_dapm_widgets),
	.dapm_routes = tfa9890_right_dapm_routes,
	.num_dapm_routes = ARRAY_SIZE(tfa9890_right_dapm_routes),
};
